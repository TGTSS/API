...
// Rota específica para documentos (mais robusta)
app.get("/api/documentos/:filename", (req, res) => {
  try {
    const { filename } = req.params;
    const filePath = path.join("public", "uploads", "documentos", filename);
    ...
    const stats = fs.statSync(filePath);
    console.log("Arquivo encontrado, tamanho:", stats.size);
